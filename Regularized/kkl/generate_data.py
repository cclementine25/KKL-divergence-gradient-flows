...
def othershapes(data, m):
  ...
  if data == "swissroll":
    data = sklearn.datasets.make_swiss_roll(n_samples=m, noise=1.0)[0]
    ...
    data = sklearn.datasets.make_circles(
        n_samples=m, factor=.5, noise=0.08)[0]
    data = data.astype("float32")
    data *= 3
    return data

  elif data == "moons":
    data = sklearn.datasets.make_moons(n_samples=m, noise=0.1)[0]
    data = data.astype("float32")
    ...
    x1 = np.random.rand(m) * 4 - 2
    x2_ = np.random.rand(m) - np.random.randint(0, 2, m) * 2
